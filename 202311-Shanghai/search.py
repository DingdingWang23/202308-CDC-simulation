...
input_folder = r'C:\\Users\\丁丁\\Desktop\\上海住建测评\\for_reasoning\\'
id2nodes = {}
        ...
        with open(input_filepath, 'r', encoding='UTF-8') as file:
            ...
            for line in lines:
                ...
                if len(line) > 1:
                    nodes.append(line[1])
            print(nodes)
            id2nodes[filename.replace(".tsv","")] = nodes

print(id2nodes)

df = pd.read_csv(r"C:\\Users\\丁丁\\Desktop\\df_1110.csv", encoding="gbk")
tuple_list = [tuple(row) for row in df.values.tolist()]

df = pd.read_csv(r"C:\\Users\\丁丁\\Desktop\\node_1110.csv", encoding="utf-8")
print(df)
node_list = df.iloc[:, 0].tolist()

print(node_list)

output_folder_p = r'C:\\Users\\丁丁\\Desktop\\上海住建测评\\p\\'
output_folder_q = r'C:\\Users\\丁丁\\Desktop\\上海住建测评\\q\\'

search_output_file = r'C:\\Users\\丁丁\\Desktop\\search_1110.csv'
...
with open(search_output_file, 'a', newline='', encoding='utf-8-sig') as csvfile:
    ...
    for item in id2nodes:
        ...
        print(q_list)
        if len(p_list)>0:
            writer.writerow([item, '重复访问', count_p/len(p_list)])
            writer.writerow([item, '向外搜索', count_q_pos / len(q_list)])
            writer.writerow([item, '向内搜索', count_q_neg / len(q_list)])
        else:
            writer.writerow([item, '重复访问', 0])
            writer.writerow([item, '向外搜索', 0])
            writer.writerow([item, '向内搜索', 0])
