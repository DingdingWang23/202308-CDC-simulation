...
input_folder = r"C:\Users\丁丁\Desktop\深圳演练\每个人的不确定性节点"

output_folder = r"C:\Users\丁丁\Desktop\深圳演练\不确定性节点输出"
...
for file_name in file_list:
    if file_name.endswith(".txt"):
        ...
        uncertainty_phrase = fuzzy_match(total_sub_sentences, node_list)

        # 生成输出文件路径
        base_name = os.path.splitext(file_name)[0]
        uncertainty_output_file = os.path.join(output_folder, "uncertainty_" + base_name + ".tsv")

        with open(uncertainty_output_file, "w+", encoding="utf-8") as f:
            for i in range(len(uncertainty_phrase)):
                print(second_list[(i // 10)], "\t", total_sub_sentences[(i // 10)], "\t", uncertainty_phrase[i],
                      file=f)
